...
RSpec.describe 'Show DRO' do
  ...
  def build_presenter(title:)
    cocina_object = build(:dro_with_metadata, id: druid, admin_policy_id: apo_druid)
                    .new(
                      structural: { isMemberOf: [collection_druid] },
                      description: {
                        title: [{ value: title }],
                        contributor: [{ name: [{ value: 'A. Contributor' }] }],
  ...
  before do
    ...
    allow(Sdr::TitleService).to receive(:call).with(druid: collection_druid).and_return('My Collection')

    sign_in(create(:user))
  end

  it 'displays the object' do
    # Defining the presenter inline because going to change the title to test refresh.
    ...
    within(find_table_value_cell('overview-table', 'Collection')) do
      ...
      expect(page).to have_link('All objects with this collection',
                                href: search_path(Search::Fields::COLLECTION_DRUIDS => [collection_druid]))
    end
    ...
    expect(page).to have_table_value('description-table', 'Contributor', 'A. Contributor')

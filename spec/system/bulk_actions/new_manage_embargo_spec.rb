...
RSpec.describe 'Create a new manage embargo bulk action' do
  let!(:user) { create(:user) }

  let(:bulk_action_label) { BulkActions::MANAGE_EMBARGO.label }
  ...
  context 'when a valid CSV file is provided' do
    it 'submits a manage embargo bulk action' do
      ...
      instructions_xpath = "//input[@type='file']/ancestor::div[1]/following-sibling::p" \
                           "[contains(normalize-space(.), 'The spreadsheet must include columns for:')]"
      expect(page).to have_xpath(upload_field_xpath)
      expect(page).to have_xpath(instructions_xpath)
      attach_file 'Upload a CSV or Excel file', 'spec/fixtures/files/manage_embargo.csv'
      ...
      expect(bulk_action.action_type).to eq(BulkActions::MANAGE_EMBARGO.action_type.to_s)
      expect(bulk_action.description).to eq('Update embargoes for test items')
      expect(bulk_action.user).to eq(user)
      expect(bulk_action.queued?).to be true

      expect(BulkActions::ManageEmbargoJob)
  ...
  context 'with an invalid file' do
    it 'shows an error message' do
      ...
      expect(page).to have_css('h1', text: bulk_action_label)
      attach_file 'Upload a CSV or Excel file', 'spec/fixtures/files/invalid_manage_embargo.csv'

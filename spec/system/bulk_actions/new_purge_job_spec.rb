...
RSpec.describe 'Create a new purge bulk action' do
  ...
  let(:bulk_action_label) { BulkActions::PURGE.label }
  ...
  context 'when a list of druids is provided' do
    it 'submits a purge bulk action' do
      ...
      fill_in 'Describe this bulk action', with: 'Purge test items'
      ...
      expect(bulk_action.action_type).to eq(BulkActions::PURGE.action_type.to_s)
      expect(bulk_action.description).to eq('Purge test items')
      expect(bulk_action.user).to eq(user)
      expect(bulk_action.queued?).to be true

      expect(BulkActions::PurgeJob).to have_been_enqueued.with(druids:, bulk_action:)
    end
  end
end

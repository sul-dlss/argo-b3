...
RSpec.describe 'Create a new register bulk action' do
  ...
  let(:bulk_action_label) { BulkActions::REGISTER_CSV.label }
  ...
  context 'when a valid CSV file is provided' do
    it 'submits a register bulk action' do
      ...
      instructions_xpath = "//input[@type='file']/ancestor::div[1]/following-sibling::p" \
                           "[contains(normalize-space(.), 'The spreadsheet must include the following columns.')]"
      expect(page).to have_xpath(upload_field_xpath)
      expect(page).to have_xpath(instructions_xpath)
      expect(page).to have_css('li', text: 'title (required unless a folio_instance_hrid has been entered)')
      ...
      expect(bulk_action.action_type).to eq(BulkActions::REGISTER_CSV.action_type.to_s)
      ...
      expect(BulkActions::RegisterCsvJob)
  ...
  context 'with an invalid file' do
    it 'shows an error message' do
      visit new_bulk_actions_register_csv_path

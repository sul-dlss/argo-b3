...
RSpec.describe 'Create a new republish bulk action' do
  ...
  let(:bulk_action_label) { BulkActions::REPUBLISH.label }
  ...
  context 'when a list of druids is provided' do
    it 'submits a republish bulk action' do
      ...
      fill_in 'Describe this bulk action', with: 'Republish test items'
      ...
      expect(bulk_action.action_type).to eq(BulkActions::REPUBLISH.action_type.to_s)
      expect(bulk_action.description).to eq('Republish test items')
      expect(bulk_action.user).to eq(user)
      expect(bulk_action.queued?).to be true

      expect(BulkActions::RepublishJob).to have_been_enqueued.with(druids:, bulk_action:)
    end
  end
end

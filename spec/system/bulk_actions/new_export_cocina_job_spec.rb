...
RSpec.describe 'Create a new export cocina json bulk action' do
  ...
  let(:bulk_action_label) { BulkActions::EXPORT_COCINA_JSON.label }
  ...
  context 'when a list of druids is provided' do
    it 'submits a export cocina json bulk action' do
      ...
      fill_in 'Describe this bulk action', with: 'Export Cocina JSON test items'
      ...
      expect(bulk_action.action_type).to eq(BulkActions::EXPORT_COCINA_JSON.action_type.to_s)
      expect(bulk_action.description).to eq('Export Cocina JSON test items')
      expect(bulk_action.user).to eq(user)
      expect(bulk_action.queued?).to be true

      expect(BulkActions::ExportCocinaJsonJob).to have_been_enqueued.with(druids:, bulk_action:)
    end
  end
end

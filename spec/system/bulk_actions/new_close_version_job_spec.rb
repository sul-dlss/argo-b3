...
RSpec.describe 'Create a new close version bulk action' do
  ...
  let(:bulk_action_label) { BulkActions::CLOSE_VERSION.label }
  ...
  context 'when a list of druids is provided' do
    it 'submits a close version bulk action' do
      ...
      fill_in 'Describe this bulk action', with: 'Close version test items'
      ...
      expect(bulk_action.action_type).to eq(BulkActions::CLOSE_VERSION.action_type.to_s)
      expect(bulk_action.description).to eq('Close version test items')
      expect(bulk_action.user).to eq(user)
      expect(bulk_action.queued?).to be true

      expect(BulkActions::CloseVersionJob).to have_been_enqueued.with(druids:, bulk_action:)
    end
  end
end

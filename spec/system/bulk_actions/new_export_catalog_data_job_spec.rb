...
RSpec.describe 'Create a new export catalog data bulk action' do
  ...
  let(:bulk_action_label) { BulkActions::EXPORT_CATALOG_DATA.label }
  ...
  context 'when a list of druids is provided' do
    it 'submits an export catalog data bulk action' do
      ...
      fill_in 'Describe this bulk action', with: 'Export catalog data test items'
      ...
      expect(bulk_action.action_type).to eq(BulkActions::EXPORT_CATALOG_DATA.action_type.to_s)
      expect(bulk_action.description).to eq('Export catalog data test items')
      expect(bulk_action.user).to eq(user)
      expect(bulk_action.queued?).to be true

      expect(BulkActions::ExportCatalogDataJob).to have_been_enqueued.with(druids:, bulk_action:)
    end
  end
end

...
RSpec.describe 'Create a new redeposit bulk action' do
  ...
  let(:bulk_action_label) { BulkActions::REDEPOSIT.label }
  ...
  context 'when a list of druids is provided' do
    it 'submits a redeposit bulk action' do
      visit new_bulk_action_path

      click_link bulk_action_label

      expect(page).to have_css('h1', text: bulk_action_label)
      expect(page).to have_current_path(new_bulk_actions_redeposit_path)
      fill_in 'Enter druid list', with: druids.join("\n")

      fill_in 'Describe this bulk action', with: 'Redeposit test items'
      ...
      expect(bulk_action.action_type).to eq(BulkActions::REDEPOSIT.action_type.to_s)
      expect(bulk_action.description).to eq('Redeposit test items')
      expect(bulk_action.user).to eq(user)
      expect(bulk_action.queued?).to be true

      expect(BulkActions::RedepositJob).to have_been_enqueued.with(druids:, bulk_action:)
    end
  end
end

...
RSpec.describe 'Create a new apply APO defaults bulk action' do
  ...
  let(:bulk_action_label) { BulkActions::APPLY_APO_DEFAULTS.label }
  ...
  context 'when a list of druids is provided' do
    it 'submits an apply APO defaults bulk action' do
      ...
      fill_in 'Describe this bulk action', with: 'Apply APO defaults to test items'
      ...
      expect(bulk_action.action_type).to eq(BulkActions::APPLY_APO_DEFAULTS.action_type.to_s)
      expect(bulk_action.description).to eq('Apply APO defaults to test items')
      expect(bulk_action.user).to eq(user)
      expect(bulk_action.queued?).to be true

      expect(BulkActions::ApplyApoDefaultsJob).to have_been_enqueued.with(druids:, bulk_action:, close_version: true)
    end
  end
end

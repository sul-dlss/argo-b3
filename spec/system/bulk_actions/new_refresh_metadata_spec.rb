...
RSpec.describe 'Create a new refresh metadata bulk action' do
  ...
  let(:bulk_action_label) { BulkActions::REFRESH_METADATA.label }
  ...
  context 'when a list of druids is provided' do
    it 'submits a refresh metadata bulk action' do
      ...
      fill_in 'Describe this bulk action', with: 'Refresh metadata for test items'
      ...
      expect(bulk_action.action_type).to eq(BulkActions::REFRESH_METADATA.action_type.to_s)
      expect(bulk_action.description).to eq('Refresh metadata for test items')
      expect(bulk_action.user).to eq(user)
      expect(bulk_action.queued?).to be true

      expect(BulkActions::RefreshMetadataJob).to have_been_enqueued.with(druids:, bulk_action:, close_version: true)
    end
  end
end

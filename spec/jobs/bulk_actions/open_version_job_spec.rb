...
RSpec.describe BulkActions::OpenVersionJob do
  subject(:job) { described_class.new(bulk_action:, druids: [druid], version_description:) }
  ...
  it 'performs the job' do
    ...
    expect(Sdr::VersionService).to have_received(:openable?).with(druid:)
    expect(Sdr::VersionService).to have_received(:open)
      .with(druid:,
  ...
  context 'when not openable' do
    before do
      allow(Sdr::VersionService).to receive(:openable?).and_return(false)
    end
  ...
  context 'when the user lacks update ability' do
    ...
    it 'does not open the version' do
      ...
      expect(Sdr::VersionService).not_to have_received(:openable?)
      expect(Sdr::VersionService).not_to have_received(:open)
    end
  end
end

...
RSpec.describe BulkActions::ReindexJob do
  ...
  describe '#perform' do
    ...
    context 'when there are errors with individual druids' do
      ...
      before do
        ...
        allow(object_client_timeout).to receive(:reindex).and_raise(Dor::Services::Client::ConnectionFailed)
      end

      it 'increments the counts, keeps running even if an individual update fails, and logs status of each update' do
        job.perform_now
        ...
        expect(log.string).to include 'Failed Dor::Services::Client::ConnectionFailed ' \

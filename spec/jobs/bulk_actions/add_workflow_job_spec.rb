...
RSpec.describe BulkActions::AddWorkflowJob do
  subject(:job) { described_class.new(bulk_action:, druids: [druid], workflow_name:, close_version: false) }
  ...
  it 'performs the job' do
    ...
    expect(Sdr::WorkflowService).to have_received(:workflow_active?).with(druid:, wf_name: workflow_name, version:)
    expect(job_item).to have_received(:open_new_version_if_needed!).with(description: 'Started accessionWF')
    expect(object_client).to have_received(:workflow).with(workflow_name)
    expect(wf_client).to have_received(:create).with(version:)
  ...
  context 'when there is an active workflow' do
    before do
      allow(Sdr::WorkflowService).to receive(:workflow_active?).and_return(true)
    end

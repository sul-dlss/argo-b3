...
RSpec.describe BulkActions::TextExtractionJob do
  ...
  let(:text_extraction) { instance_double(Sdr::TextExtraction, start: true, possible?: true, wf_name: 'ocrWF') }

  before do
    allow(described_class::JobItem).to receive(:new).and_return(job_item)
    allow(Sdr::VersionService).to receive_messages(open?: false, assembling?: false)
  ...
  it 'performs the job' do
    ...
    expect(Sdr::VersionService).to have_received(:open?).with(druid:)
  ...
  context 'when the object is currently assembling' do
    before do
      allow(Sdr::VersionService).to receive(:assembling?).and_return(true)
    end

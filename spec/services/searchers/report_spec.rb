...
RSpec.describe Searchers::Report do
  ...
  context 'when streaming results' do
    subject(:results) { described_class.call(search_form:, fields:, stream:, rows: 10, workgroups:) }
  ...
  context 'when not streaming results' do
    subject(:results) { described_class.call(search_form:, fields:, rows: 10, workgroups:) }
  ...
  context 'with specific permission targets', :solr do
    ...
    it 'filters reports generated from a search to only readable items' do
      report = described_class.call(search_form:, fields: [Search::Fields::ID], rows: 10, workgroups:)

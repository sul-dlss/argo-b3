...
RSpec.describe Sdr::TextExtraction do
  ...
  before do
    allow(Settings.feature_flags).to receive_messages(ocr_workflow: true, speech_to_text_workflow: true)
  end
  ...
  describe '#possible?' do
    ...
    context 'when the object is media' do
      ...
      context 'when speech_to_text_workflow is disabled' do
        before do
          allow(Settings.feature_flags).to receive(:speech_to_text_workflow).and_return(false)
        end
    ...
    context 'when the object is already opened' do
      ...
      context 'when the object is media' do
        ...
        it 'starts speechToTextWF and returns true' do
          expect(text_extraction.start).to be true
          expect(fake_workflow_client).to have_received(:create).with(version:, lane_id: 'low',
                                                                      context: speech_to_text_context)
        end
      end
    end

...
RSpec.describe DescriptiveCsv::DescriptionsGrouper do
  ...
  let(:descriptions) do
    {
      druid1 => DescriptiveCsv::Export.export(description: description1, source_id: ''),
      druid2 => DescriptiveCsv::Export.export(description: description2, source_id: ''),
      druid3 => DescriptiveCsv::Export.export(description: description3, source_id: '')
    }
  end
  ...
  context 'with a more complex example containing multiple repeat values' do
    let(:descriptions) do
      {
        ...
        druid2 => DescriptiveCsv::Export.export(description: description2, source_id: ''),
        druid3 => DescriptiveCsv::Export.export(description: description3, source_id: ''),
        druid4 => DescriptiveCsv::Export.export(description: description4, source_id: ''),
        druid5 => DescriptiveCsv::Export.export(description: description5, source_id: ''),
        druid6 => DescriptiveCsv::Export.export(description: description6, source_id: '')
      }
    end
    ...
    it 'groups forms as expected' do
      ...
      expect(run.dig(druid3, 'form5.value'))
        .to eq('1 computer disc : CD-ROM ; 4 3/4 in. + 1 user guide (12 leaves : illustrations ; 28 cm.)')
      ...
      expect(run.dig(druid4, 'form5.value'))
        .to eq('1 computer disc ; 4 3/4 in. + 1 volume (9 leaves : illustrations ; 28 cm)')
      ...
      expect(run.dig(druid6, 'form5.value'))
        .to eq('6 USB hard drives : 3.5 USB 3.0 to SATA III HDD enclosure with UASP support')
    ...
    it 'groups notes as expected' do
      ...
      expect(run.dig(druid2, 'note3.value'))
        .to eq('Users guide entitled: NIST surface structure database (SSD) with interactive analysis and ' \
               'visualization / authors, P.R. Watson, M.A. Van Hove, K. Herrmann.')
      expect(run.dig(druid2, 'note4.value'))
        .to eq('Provides extensive structural information about surface structures determined from experiment.')
      ...
      expect(run.dig(druid5, 'note1.value'))
        .to eq('Description based on online resource, title from email from The New York Times Business ' \
               'Development (Stanford University Libraries, viewed November 2, 2021)')
      ...
      expect(run.dig(druid6, 'note4.value'))
        .to eq('Associated Press Collections includes decades worth of access to an array of internal AP publications and records from select AP bureaus. Contents provided by Associated Press Corporate Archives, AP Images, and AP Archive.') # rubocop:disable Layout/LineLength
  ...
  context 'with notes containing multiple repeat values including those with no displayLabel or type' do
    ...
    it 'groups notes as expected' do
      ...
      expect(run.dig(druid5, 'note2.value')).to be_nil
      expect(run.dig(druid5, 'note3.value')).to be_nil
      expect(run.dig(druid5, 'note4.value'))
        .to eq('40-Year Textual Digital Archive of nytimes.com, initially: 1/1/1980-12/31/2020, which consists of all available articles (approximately 3,000,000) published by The New York Times, including but not limited to news, lifestyle, opinion and The New York Times Magazine, and excludes reader comments, paid obituaries and the kids section. The archives are packed by year, and the naming convention is nyt_nitf_YYYY.tar.gz. Inside the tar archive will be one folder per month, named YYYYMM, and inside of those folders will be individual files for each story moved that year. The naming convention for the stories is YYYY and then an internal ID number. Stories are marked up using News Industry Text Format (NITF 3.3).') # rubocop:disable Layout/LineLength
      expect(run.dig(druid5, 'note4.type')).to eq('abstract')
      expect(run.dig(druid5, 'note5.value')).to be_nil
      expect(run.dig(druid5, 'note6.value')).to be_nil
      expect(run.dig(druid5, 'note7.value')).to be_nil
  ...
  context 'with all notes lacking displayLabel or type' do
    ...
    it 'groups notes with no empty columns at start of row' do
      ...
      expect(run.dig(druid2, 'note3.value'))
        .to eq('Three Provides extensive structural information about surface structures determined from experiment.')
    end
  end
  ...
  context 'with a forms that are varlues only' do
    let(:descriptions) do
      {
        druid1 => DescriptiveCsv::Export.export(description:, source_id: '')
      }
    end

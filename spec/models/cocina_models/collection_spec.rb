...
RSpec.describe CocinaModels::Collection do
  ...
  describe '#initialize' do
    context 'with a valid Cocina::Models::CollectionWithMetadata' do
      it 'initializes with a Cocina::Models::CollectionWithMetadata' do
        ...
        expect(collection.source_id).to eq(cocina_object.identification.sourceId)
      end
    end
    ...
    context 'with an invalid object' do
      ...
      it 'raises an error if initialized with an invalid object' do
        expect { collection }.to raise_error(ArgumentError)
      end
    end
  end
  ...
  describe '#save!' do
    ...
    context 'with valid and changed attributes' do
      ...
      before do
        collection.source_id = new_source_id
      end

      it 'saves the model successfully' do
        collection.save!(user_name:, description:)
  ...
  describe 'type predicates' do
    it 'returns false for #dro?' do
      expect(collection.dro?).to be false
    end

    it 'returns true for #collection?' do
      expect(collection.collection?).to be true
    end

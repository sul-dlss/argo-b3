...
# Build stage only: CSS is compiled during assets:precompile, so the runtime image doesn't need Node.
...
# node_modules is only needed to compile the CSS, so drop it to keep it out of the final image.

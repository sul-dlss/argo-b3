...
module Sdr
  ...
  class TextExtraction
    RESOURCE_TYPE_WORKFLOWS = {
      Cocina::Models::ObjectType.book => 'ocrWF',
      Cocina::Models::ObjectType.document => 'ocrWF',
      Cocina::Models::ObjectType.image => 'ocrWF',
      Cocina::Models::ObjectType.media => 'speechToTextWF'
    }.freeze
    ...
    def start
      ...
      # if the object is already open, they are the same; if not, ocrWF/speechToTextWF will open the object version,
      # incrementing it
      version += 1 unless already_opened
      Dor::Services::Client.object(cocina_object.externalIdentifier).workflow(wf_name).create(version:, lane_id: 'low',
                                                                                              context:)
    end
    ...
    # mapping of cocina resource types to workflow names, filtered by enabled feature flags
    def resource_type_mapping
      RESOURCE_TYPE_WORKFLOWS.select do |_type, wf_name|
        (wf_name == 'ocrWF' && Settings.feature_flags.ocr_workflow) ||
          (wf_name == 'speechToTextWF' && Settings.feature_flags.speech_to_text_workflow)
      end
    end

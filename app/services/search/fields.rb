# frozen_string_literal: true

module Search
  # Constants for Solr fields
  module Fields
    ACCESS_RIGHTS = 'rights_descriptions_ssimdv'
    AGREEMENT_DRUID = 'agreement_ssi'
    AGREEMENT_TITLE = 'agreement_ssidv'
    ALL_TAGS = 'tag_ssim'
    APO_DRUID = 'governed_by_ssim'
    APO_TITLE = 'apo_title_ssimdv'
    APO_TITLE_DRUID = 'apo_title_druid_ssimdv'
    AUTHOR = 'author_display_ss'
    BARCODES = 'barcode_id_ssimdv'
    BARE_APO_DRUID = 'bare_governed_by_ss'
    BARE_COLLECTION_DRUID = 'bare_member_of_collection_ssm'
    BARE_DRUID = 'druid_bare_ssi'
    CATALOG_RECORD_ID = 'folio_instance_hrid_ssim'
    COLLECTION_DRUIDS = 'member_of_collection_ssim'
    COLLECTION_TITLES = 'collection_title_ssimdv'
    COLLECTION_TITLE_DRUIDS = 'collection_title_druid_ssimdv'
    CONSTITUENTS_COUNT = 'constituents_count_ips'
    CONTENT_TYPES = 'content_type_ssimdv'
    DISSERTATION_ID = 'dissertation_id_ss'
    DOI = 'doi_ssimdv'
    EARLIEST_ACCESSIONED_DATE = 'accessioned_earliest_dtpsidv'
    EARLIEST_REGISTERED_DATE = 'registered_earliest_dtpsidv'
    EMBARGO_RELEASE_DATE = 'embargo_release_dtpsimdv'
    FILE_COUNT = 'content_file_count_itsi'
    FILE_ROLES = 'content_file_roles_ssimdv'
    FIRST_SHELVED_IMAGE = 'first_shelved_image_ss'
    FORMATS = 'format_ssimdv'
    FORMATTED_DEPOSITED_LATEST_DATE = 'formatted_deposited_latest_ss'
    FORMATTED_EARLIEST_ACCESSIONED_DATE = 'formatted_accessioned_earliest_ss'
    FORMATTED_EMBARGO_RELEASE_DATE = 'formatted_embargo_release_ss'
    FORMATTED_PUBLISHED_EARLIEST_DATE = 'formatted_published_earliest_ss'
    FORMATTED_REGISTERED_EARLIEST_DATE = 'formatted_registered_earliest_ss'
    GENRES = 'genre_ssimdv'
    HUMAN_PRESERVED_SIZE = 'human_preserved_size_ss'
    ID = 'id'
    IDENTIFIERS = 'identifier_ssim'
    LANGUAGES = 'sw_language_names_ssimdv'
    LAST_ACCESSIONED_DATE = 'accessioned_latest_dtpsidv'
    LAST_DEPOSITED_DATE = 'deposited_latest_dtpsidv'
    LAST_OPENED_DATE = 'opened_latest_dtpsidv'
    LAST_PUBLISHED_DATE = 'published_latest_dtpsidv'
    LICENSES = 'use_license_machine_ssidv'
    METADATA_SOURCE = 'metadata_source_ssimdv'
    MIMETYPES = 'content_file_mimetypes_ssimdv'
    MODS_RESOURCE_TYPES = 'mods_typeOfResource_ssimdv'
    OBJECT_TYPES = 'objectType_ssimdv'
    ORCIDS = 'contributor_orcids_ssimdv'
    OTHER_TAGS_EXPLODED = 'exploded_nonproject_tag_ssimdv'
    OTHER_HIERARCHICAL_TAGS = 'hierarchical_other_tag_ssimdv'
    PRESERVATION_SIZE = 'preserved_size_lpsidv'
    PUBLICATION_CREATED_DATE = 'originInfo_date_created_tesim'
    PUBLICATION_DATE = 'publication_year_ssidv'
    PUBLICATION_PLACE = 'originInfo_place_placeTerm_tesim'
    PUBLISHER = 'originInfo_publisher_tesim'
    PURL = 'purl_ss'
    PROCESSING_STATUS = 'processing_status_text_ssidv'
    PROJECTS = 'project_tag_ssim'
    PROJECTS_EXPLODED = 'exploded_project_tag_ssimdv'
    PROJECTS_HIERARCHICAL = 'hierarchical_project_tag_ssimdv'
    REGIONS = 'subject_place_ssimdv'
    REGISTERED_BY = 'registered_by_tag_ssim'
    REGISTERED_DATE = 'registered_dtpsimdv'
    RELEASED_TO = 'released_to_ssim'
    RELEASED_TO_EARTHWORKS = 'released_to_earthworks_dtpsidv'
    RELEASED_TO_PURL_SITEMAP = 'released_to_purl_sitemap_dtpsidv'
    RELEASED_TO_SEARCHWORKS = 'released_to_searchworks_dtpsidv'
    RESOURCE_COUNT = 'resource_count_itsi'
    SHELVED_FILE_COUNT = 'shelved_content_file_count_itsi'
    SOURCE_ID = 'source_id_ssi'
    STATUS = 'status_ssi'
    TICKETS = 'ticket_tag_ssim'
    TITLE = 'display_title_ss'
    TOPICS = 'subject_topic_other_ssimdv'
    VERSION = 'current_version_ipsidv'
    WORKFLOW_ERRORS = 'wf_error_ssim'
    WPS_HIERARCHICAL_WORKFLOWS = 'wf_hierarchical_wps_ssimdv'
    WPS_WORKFLOWS = 'wf_wps_ssimdv'
  end
end

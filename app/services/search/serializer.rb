...
module Search
  ...
  class Serializer
    ...
    def facet_part_for(facet_config:, exclude: false)
      ...
      values = search_form.send(form_field).map { |value| quote(value) }
      combine_label_and_values(facet_config:, values:, exclude:)
    end

    def dynamic_facet_part_for(facet_config:) # rubocop:disable Metrics/AbcSize
      date_from_value = date_from_value_for(facet_config:)
      date_to_value = date_to_value_for(facet_config:)
      form_values = search_form.send(facet_config.form_field)

      return if form_values.blank? && date_from_value.blank? && date_to_value.blank?

      values = form_values.map { |value| quote(facet_value_label(value)) }
      values << combine_dates(date_from_value:, date_to_value:) if date_from_value.present? || date_to_value.present?
    ...
    def date_from_value_for(facet_config:)
      ...
      search_form.send(facet_config.date_from_form_field)
    end

    def date_to_value_for(facet_config:)
      return if facet_config.date_to_form_field.blank?

      search_form.send(facet_config.date_to_form_field)
    end

...
module DescriptiveCsv
  ...
  class NotesGrouper
    ...
    def group # rubocop:disable Metrics/AbcSize, Metrics/MethodLength, Metrics/PerceivedComplexity, Metrics/CyclomaticComplexity
      descriptions.transform_values do |description| # rubocop:disable Metrics/BlockLength
        ...
        description.transform_keys! do |key| # rubocop:disable Metrics/BlockLength
          ...
          label_for_note_number = description.slice(*description.keys.grep(/^old_note#{note_number[1]}\.displayLabel/))
                                             .values.first
          # Get the type value of the note number corresponding to the key currently being transformed
          type_for_note_number = description.slice(*description.keys.grep(/^old_note#{note_number[1]}\.type/))
                                            .values.first
          ...
          new_note_number = case description.slice(*description.keys.grep(/note.+(displayLabel|type)/))
                                            ...
                                            .count do |_key, value|
                                              ...
                                              [hash["old_note#{num}.displayLabel"],
                                               hash["old_note#{num}.type"]] == [label_for_note_number,
                                                                                type_for_note_number] ||
                                                [hash["note#{num}.displayLabel"],
                                                 hash["note#{num}.type"]] == [label_for_note_number,
                                                                              type_for_note_number]
                                 end
    ...
    def ordered_mapping_from(descriptions) # rubocop:disable Metrics/AbcSize, Metrics/MethodLength, Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity
      ...
      # [
      #   [[nil, "statement of responsibility"], [nil, nil], [nil, "system details"], ["Display label", nil], ["Another label", "condition"], ["Display label", nil]], # rubocop:disable Layout/LineLength
      #   [[nil, "abstract"], [nil, nil], [nil, nil], [nil, nil], [nil, "system details"], ["Display label", nil], ["Another label", "condition"]], # rubocop:disable Layout/LineLength
      ...
      #  "note5"=>["Display label", nil], "note6"=>["Display label", nil], "note7"=>[nil, "statement of responsibility"], # rubocop:disable Layout/LineLength

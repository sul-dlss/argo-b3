...
module DescriptiveCsv
  ...
  class Validator # rubocop:disable Metrics/ClassLength
    ...
    def validate_title_types_and_values # rubocop:disable Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity, Metrics/AbcSize
      @headers.filter_map { |header| header&.match(/\Atitle(\d+)\.type\z/) }.each do |matcher|
        ...
        @csv.each do |row|
          next if row[title_type_header].blank? &&
                  row[title_structured_value_header].blank? &&
                  row[title_value_header].blank?

          if row[title_value_header].blank? && row[title_structured_value_header].blank?
    ...
    def validate_structured_title_types_and_values # rubocop:disable Metrics/CyclomaticComplexity, Metrics/PerceivedComplexity, Metrics/AbcSize
      @headers.filter_map { |header| header&.match(/\Atitle(\d+)\.structuredValue(\d+)\.type\z/) }.each do |matcher|
        ...
        @csv.each do |row|
          ...
          if row[title_structured_value_header].blank?
            @errors << "Missing title value for #{title_structured_type_header}."
          end
          if row[title_structured_type_header].blank?
            @errors << "Missing title type for #{title_structured_value_header}."
          end
        end
      end
    end
    ...
    def title_parallel_value_header?
      @headers.include?('title1.parallelValue1.value') ||
        @headers.include?('title1.parallelValue1.structuredValue1.value')
    end
